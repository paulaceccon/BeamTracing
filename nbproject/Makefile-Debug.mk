#
# Generated Makefile - do not edit!
#
# Edit the Makefile in the project folder instead (../Makefile). Each target
# has a -pre and a -post target defined where you can add customized code.
#
# This makefile implements configuration specific macros and targets.


# Environment
MKDIR=mkdir
CP=cp
GREP=grep
NM=nm
CCADMIN=CCadmin
RANLIB=ranlib
CC=gcc
CCC=g++
CXX=g++
FC=gfortran
AS=as

# Macros
CND_PLATFORM=GNU-MacOSX
CND_DLIB_EXT=dylib
CND_CONF=Debug
CND_DISTDIR=dist
CND_BUILDDIR=build

# Include project Makefile
include Makefile

# Object Directory
OBJECTDIR=${CND_BUILDDIR}/${CND_CONF}/${CND_PLATFORM}

# Object Files
OBJECTFILES= \
	${OBJECTDIR}/Environment.o \
	${OBJECTDIR}/MathUtils.o \
	${OBJECTDIR}/Ray.o \
	${OBJECTDIR}/Receiver.o \
	${OBJECTDIR}/Room.o \
	${OBJECTDIR}/Source.o \
	${OBJECTDIR}/Tree.o \
	${OBJECTDIR}/TreeNode.o \
	${OBJECTDIR}/Wall.o \
	${OBJECTDIR}/main.o


# C Compiler Flags
CFLAGS=

# CC Compiler Flags
CCFLAGS=-Wno-deprecated
CXXFLAGS=-Wno-deprecated

# Fortran Compiler Flags
FFLAGS=

# Assembler Flags
ASFLAGS=

# Link Libraries and Options
LDLIBSOPTIONS=

# Build Targets
.build-conf: ${BUILD_SUBPROJECTS}
	"${MAKE}"  -f nbproject/Makefile-${CND_CONF}.mk ${CND_DISTDIR}/${CND_CONF}/${CND_PLATFORM}/beamtracing

${CND_DISTDIR}/${CND_CONF}/${CND_PLATFORM}/beamtracing: ${OBJECTFILES}
	${MKDIR} -p ${CND_DISTDIR}/${CND_CONF}/${CND_PLATFORM}
	${LINK.cc} -o ${CND_DISTDIR}/${CND_CONF}/${CND_PLATFORM}/beamtracing ${OBJECTFILES} ${LDLIBSOPTIONS} -framework OpenGL -framework GLUT

${OBJECTDIR}/Environment.o: Environment.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/Environment.o Environment.cpp

${OBJECTDIR}/MathUtils.o: MathUtils.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/MathUtils.o MathUtils.cpp

${OBJECTDIR}/Ray.o: Ray.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/Ray.o Ray.cpp

${OBJECTDIR}/Receiver.o: Receiver.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/Receiver.o Receiver.cpp

${OBJECTDIR}/Room.o: Room.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/Room.o Room.cpp

${OBJECTDIR}/Source.o: Source.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/Source.o Source.cpp

${OBJECTDIR}/Tree.o: Tree.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/Tree.o Tree.cpp

${OBJECTDIR}/TreeNode.o: TreeNode.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/TreeNode.o TreeNode.cpp

${OBJECTDIR}/Wall.o: Wall.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/Wall.o Wall.cpp

${OBJECTDIR}/main.o: main.cpp 
	${MKDIR} -p ${OBJECTDIR}
	${RM} "$@.d"
	$(COMPILE.cc) -g -I/System/Library/Frameworks -MMD -MP -MF "$@.d" -o ${OBJECTDIR}/main.o main.cpp

# Subprojects
.build-subprojects:

# Clean Targets
.clean-conf: ${CLEAN_SUBPROJECTS}
	${RM} -r ${CND_BUILDDIR}/${CND_CONF}
	${RM} ${CND_DISTDIR}/${CND_CONF}/${CND_PLATFORM}/beamtracing

# Subprojects
.clean-subprojects:

# Enable dependency checking
.dep.inc: .depcheck-impl

include .dep.inc
